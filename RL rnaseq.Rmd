...
This expression will identify genes that have ZERO reads across all samples. In our case its ~40% of all genes.
```{r}
...
We set our threshold to cpm>1 in at least one sample due to our limitation of one replicate. Remember that cpm is a function of library size, so the raw read count threshold is unique per sample. Following this action, we will reduce our total genes down significantly, and the library sizes will change as well.
```{r}
